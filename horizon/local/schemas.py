...
class ListRoleAssignmentsPagination(BaseSchema):
    ...
    per_page: int = Field(
        10, ge=1, le=100, description="The number of items to return per page"
    )


class ListRoleAssignmentsPDPBody(BaseSchema):
    filters: ListRoleAssignmentsFilters = Field(
        ..., description="The filters to apply to the list"
    )
    pagination: ListRoleAssignmentsPagination = Field(
...
class RoleAssignment(BaseSchema):
    ...
    resource_instance: str | None = Field(
        None, description="the resource instance the role is associated with"
    )
